...
in
{
  ...
  home-manager.users."${config.vars.username}" = {
    wayland.windowManager.hyprland = {
      ...
      # laptop. Done here rather than with static monitor rules because an unknown
      # external has no fixed descriptor to bind to. Placement is debounced so it
      # runs once the layout (and shikane's output config) has settled after a
      # hotplug, instead of racing it — reacting to monitor.added directly reads
      # hl.get_monitors() mid-reconfiguration and places workspaces wrong.
      extraConfig = ''
        local laptop = "eDP-1"
        ...
        local function place()
          ...
          if ext then
            ...
            -- no empty one is auto-spawned, then land focus on the external. On
            -- unplug we leave focus alone (no empty workspace is created there).
        ...
        local function schedule()
          ...
          pending = hl.timer(place, { timeout = 600, type = "oneshot" })
        end
