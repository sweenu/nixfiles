...
let
  ...
  autheliaConfig = pkgs.substituteAll {
    ...
    inherit autheliaDirInContainer autheliaPort usersFilePath;
  };
in
{
  age.secrets = {
    ...
    "authelia/smtpPassword".file = "${self}/secrets/authelia/google_app_password.age";
  };
  ...
  virtualisation.arion.projects.authelia.settings = {
    ...
    services.authelia.service = {
      ...
      expose = [ autheliaPort ];
      volumes = [
        "${autheliaDir}:${autheliaDirInContainer}"
        "${autheliaConfig}:${autheliaDirInContainer}/configuration.yml:ro"
